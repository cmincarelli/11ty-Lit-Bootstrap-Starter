...
module.exports = function (eleventyConfig) {
	...
	// Slice filter: array slice
	eleventyConfig.addFilter("slice", function (arr, start, end) {
		if (!Array.isArray(arr)) return [];
		return arr.slice(start, end);
	});

	// JSON filter: convert to JSON string (raw, not HTML encoded)
	eleventyConfig.addFilter("json", function (value) {
		return JSON.stringify(value);
	});

	// Raw JSON filter: convert to JSON string without HTML encoding
	eleventyConfig.addFilter("jsonRaw", function (value) {
		return JSON.stringify(value);
	});
}